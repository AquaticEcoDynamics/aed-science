...
\begin{equation} 
\frac{\partial{NO}_3}{\partial t}=f_{nitrf} - f_{denit} + \color{\#394E73}{f_{atmdep}^{NOx}}+\color{#8B701F}{f_{sed}^{NOx}} (\#eq:nitrogen1) 
\end{equation}
...
**Oxygen limitation on nitrification:** An extra option is to allow low O~2~ concentration to limit the nitrification rate, $f(O_{2})$. If the [aed2_oxygen module](#DO_1) is linked to the nitrogen module, then this setting will switch on automatically. The limitation effect is calculated according to:
...
**pH limitation on nitrification:** The rate of nitrification can be decreased at very high and low pH values, if pH is being simulated (see [Carbon](#Carbon_1) and Geochemsitry). To use this function, the parameter `simNitrfpH` is set to `.TRUE.`. Around neutral pH, between the upper and lower optimum parameters, the reaction proceeds at the normal rate. Below a lower tolerance, and above an upper tolerance, the reaction is stopped entirely. Between the optimum and tolerance values, the rate is limited, in proportion to pH. The piecewise function is described as:
...
\begin{equation} 
f_{denit} =R_{denit} \theta_{denit}^{T-20}\frac{NO_3\^-}{NO_3^{-}+K_{NO3}}\Phi_{O2}^{denit}\left[O_2\right] NO_3^- (\#eq:nitrogen6) 
\end{equation}
