...
In most circum-neutral waters the carbonate buffer system controls the pH conditions within the water. When CO2 ($CO_{2(gas)}$) is dissolved in water, it can exist in a variety of forms. Dissolved carbon dioxide, $CO_{2_{(aq)}}$, forms carbonic acid, $H_2CO_3$, and dissociates into bicarbonate, $HCO_3^-$, and carbonate, $CO_3^{2-}$, forms.
...
kbl(carbon[,3:NCOL(carbon)], caption = "Carbon - state variables", align = "l",) %>%
  ...
  scroll_box(width = "770px", height = "405px",
             fixed_thead = FALSE)
```
