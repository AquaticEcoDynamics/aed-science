...
<iframe src ="https://gilesnknight.shinyapps.io/phytoplankton_plots" height="630px" width="100%" style="border:none;">
</iframe>

<br>

```{r phytoplanktonPlot1, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Light limitation of pytoplankton via various model approaches.", out.width = '100%'}
library(plotly)
library(dplyr)
...
# )
# 
# for(i in seq(20,510, by = 10)){
#   case6_temp <- data.frame(
#     par_c = seq(0,2000,by=10),
#     I_S = i
#   )
#   case6_temp$fI <- ((2+5)*(case6_temp$par_c/case6_temp$I_S))/(1+(5*(case6_temp$par_c/case6_temp$I_S)+((case6_temp$par_c/case6_temp$I_S)*(case6_temp$par_c/case6_temp$I_S))))
#   case6 <- bind_rows(case6,case6_temp)
# }
...
fig <- fig %>%
  layout(
    hovermode = "x unified",
    xaxis = list(title='Light (PAR)',hoverformat = "Light (PAR)"), 
    yaxis = list(title='Photosynthesis (fI)')
    ) %>% 
  animation_opts(
