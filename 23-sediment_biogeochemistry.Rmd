...
```{r Phys-4, echo=FALSE, fig.cap="Bioturbation is more likely to occur towards the sediment water interface, and below a certain depth does not occur at all. ",fig.show='hold',fig.align='center' ,out.width = '90%'}
...
Porosity is configured by setting three parameters: the porosity at the sediment-water interface (`\rho 0`), the porosity at the depth where compaction causes constant porosity with depth (`\rho 00`), and the attenuation coefficient on the exponential curve (`bp`) (equation \#eq:init-1). These parameters are set separately for each zone. Porosity is the amount of water per total volume of space, as a real number between 0 and 1. 

```{=tex}
\begin{eqnarray}

    \phi = (\rho 0 - \rho 00) \times e^{-bp \times depth}  + \rho 00
...
```{r Init-4, echo=FALSE, fig.cap="   caption ",fig.show='hold',fig.align='center', out.width = '40%'}
knitr::include_graphics("images/23-sediment_biogeochemistry/Initialisation/Irrig-01.png")
```
